...
Section 2 says LaVar reads "the way a literate adult reads." That is not enough on its own — this repo has already spent multiple passes learning specific, named ways a reading goes wrong, and LaVar's judgment about a sidecar is only as good as its knowledge of that record. This section is not a summary of `READING-POLICY.md` and `CLAUDE.md` — it is a pointer to them, plus the subset of their rules that bears directly on judging a sidecar's quality. **Before grading or producing any sidecar, LaVar must have actually read `eoreader7/READING-POLICY.md` (P0–P7) and `eoreader7/CLAUDE.md` in full — not just this section.** Duplicating their content here would drift from the source the moment either file is next amended; citing them does not.

**Name the assembly (READING-POLICY P0).** A sidecar is only as trustworthy as the pipeline that produced it. `host/corpus.js`'s `createSession`/`admitChunked`/`sessionReferents`/`sessionRelations` chain — surfaces → referents → pronoun binding → relation vocabulary, in that order — is the assembled reader. A sidecar built by a hand-chained driver (`scripts/read-*.mjs`) is an experiment asking one question, not a reading, and grading it as if it were the full assembly over-claims exactly as badly as the co-occurrence golden P0 warns about under-claiming. Every sidecar LaVar touches must say which assembly produced it before LaVar says anything about its quality.

**Check the stage, not just the output (READING-POLICY P2).** A reading is eight stages: perception, witnessed admission, alias resolution, pronoun binding, typed relation, altitude, population, kind. A sidecar that skipped population closure (stage 7) before kind induction (stage 8) will report "mentioned once" as its dominant finding — a known, named failure mode, not a fact about the material. When a sidecar looks thin, ask which stage is missing before asking whether the material is thin.

**Priors have a giver, and are injected, never derived (READING-POLICY P3).** The Marmeladov fold (§1) is exactly the failure this rule predicts: `surfaces.js`'s ENGINE-tier rule ("structural, no witness needed" — shared final token) fired in place of a MODEL-tier per-text coref prior that should have separated Sofya Semyonovna Marmeladov from her father. **LaVar's default question on any fold it doubts is not "is this wrong" but "which tier decided this, and was that the right tier."** An engine-tier structural rule deciding a question that needed a per-text prior is the specific, recurring shape of this bug class — not a one-off.

**A gap is a result, never a suppressed error (READING-POLICY P4).** `nul/index.js`'s closed `GAP_TYPES` vocabulary — `degenerate_ground`, `missing_kind_prior`, `cast_truncated`, `wayfinder.void` — are findings a sidecar should carry, not failures LaVar should try to make disappear by loosening a gate. A "clean" sidecar with zero gaps on a genuinely difficult text is more suspicious than one with several typed gaps on file.

**Byte-offset self-verification is mandatory (READING-POLICY P5.2, and this repo's CLAUDE.md `P168` note "an address is a birth, not a spelling").** A CRLF source whose offsets were computed post-normalization but sliced against raw bytes drifts one byte per line — `0/57 verified` is the recorded shape of that bug. Every address a sidecar emits must self-verify against the actual bytes at that offset; LaVar checking a revision's own address against the material (exactly what `live_priors/scripts/lavar-prior-scaffold.mjs`'s schema enforces for live priors, and what caught this session's own wrong offset while building the Alice worked example) is not optional rigor, it is the same check this codebase has already paid for once.

**State the reader's configuration beside every number (CLAUDE.md, "added 2026-09-04").** A driver that concluded two Borodino accounts share no functional relation had its admission gate open and its extraction levers off — real arithmetic, real null, wrong configuration, and the finding was about the harness, not the material. Before LaVar trusts or reports on any sidecar's numbers, it must see which gates, walls, and levers were on. A sidecar with no stated configuration is not gradable yet.

**Search for the organ before flagging a "missing" one (CLAUDE.md, "search for the organ before you write one").** Before LaVar records a `missed relation` or `missed being` finding (§9) as evidence the engine lacks a capability, grep for it — `emergence/binding.js`, `perceiver/text/relations.js`, `perceiver/text/roles.js` and siblings have repeatedly turned out to already do the thing a plausible from-scratch fix would have re-implemented, worse. A miss LaVar should actually revise is one where the organ ran and produced the wrong answer, not one where LaVar assumed no organ exists.
...
**2026-09-09, third pass — the children's-book corpus, multilingual arm.** `18-childrens-books/` landed as a pilot: 38 books across 17 languages, from two auth-free sources — `content.digitallibrary.io` (Global Digital Library and StoryWeaver, merged on one platform; the `publisher` field on each manifest entry says which catalogued a given book) and `global-asp/global-asp` (African Storybook Project translations, license and credits inline per file). This is distinct from §4's ladder, which is public-domain English-language graded material (McGuffey, Aesop, Grimm, Andersen, Potter, Alice) for measuring revision rate by rung; this corpus is breadth across languages, for whenever §8's "second language gets its own ladder with its own giver" becomes the live question. A third source, SIL's Bloom Library, was not pulled — `sil-ai/bloom-lm` on Hugging Face is gated and needs a human to accept its terms and supply a token before any fetch script can reach it. `scripts/enforce-min-words.mjs` exempts this category by directory (`EXEMPT_DIR`) rather than pruning it: a complete picture book can be 60 words and still be the whole work, which is what the 600-word floor is trying to distinguish from a fragment in the first place. Per-document source, license, and publisher live in `manifests/childrens-books-manifest.json`; the rights summary, including which titles are CC BY-NC-SA rather than plain CC BY, is in `18-childrens-books/ATTRIBUTION.md`.
