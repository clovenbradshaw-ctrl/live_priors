...
**A second, resemblance-based lens, made active rather than left as a measurement.** `READING-SPEC.md` S104: the-fold's `relative.js` Field — sparse bit-overlap, no referent identity, no addresses — recovers gold-verified content for referents the address system has under-covered at 73.5% in its own top-10%, against 14.6% for a matched random-cue control. `eval/lavar/field-lens-boost.mjs` runs it as a standing, standalone, whole-book pass: **68 new propositions across the whole book, 33 of 40 touched referents going from ZERO resolved arrangements to real, typed, address-verified content** — the Queen, the King, the Dormouse, the Hatter, the Gryphon, the Duchess, Bill, Dinah, and more, across nearly every chapter. It never asserts on its own: every candidate is re-run through the same extraction machinery the main read uses, and a find only ships if that machinery independently confirms it. Verified rather than assumed: still 100% recoverable book-wide after the append, and the four hand-built goldens' own scores are byte-identical before and after — genuinely additive, not a regression dressed as a gain. It is a SEPARATE, standalone script by design, after composing it inline with the reread mechanism in one invocation once produced something indistinguishable from a real regression until isolated. What it does NOT move: the hand-authored clause-level golden score, on this run — real referent-targeted content the boost adds sometimes segments a sentence differently than the golden's own hand-chosen clause boundary, so referent-coverage recall and golden-clause recall are two different metrics, and this pass improves the first without moving the second.
